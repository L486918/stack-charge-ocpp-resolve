 ...
 *  4. mapKey 缺失或非法时，回退为按值的运行时类型分类（Number->signals,
 *     String->extra_info, 数组->map_array），保证字段不丢。
 */
@Slf4j
public class OcppDecoderV2 implements Serializable {
    ...
    public static final class DecodeResult implements Serializable {
        ...
        private final Map<String, Object> mapArray;

        DecodeResult(int signalsCap, int extraInfoCap, int mapArrayCap) {
        ...
        public Map<String, Object> getMapArray() { return mapArray; }
    ...
    public OcppDecoderV2(String url){
    ...
    public OcppDecoderV2(List<AgreementVersion> agreementVersionList) {
    ...
    private static void put(DecodeResult r, CompiledField f, Object v) {
        ...
        switch (bucket) {
            case BUCKET_SIGNALS:
                r.signals.put(f.outKey, v);
                break;
            case BUCKET_MAP_ARRAY:
                r.mapArray.put(f.outKey, v);
                break;
            default:
                r.extraInfo.put(f.outKey, v);
        }
    }
